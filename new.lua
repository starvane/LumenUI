--[[
    MIGRASI UI: Obsidian -> LumenHub
    --------------------------------
    Yang diubah HANYA bagian UI (loading library, Window, Tab, Groupbox,
    Toggle/Input/Button/Dropdown/Slider/Keybind, Notify, Save/Config).
    Semua fungsi inti (BeatGame, ServerHop, SendWebhook, FindFinish, dll)
    disalin PERSIS tanpa perubahan logic.

    Catatan fitur Obsidian yang TIDAK ada equivalennya di LumenHub (dihapus):
      - ThemeManager (custom theme picker)
      - SetNotifySide / ShowCustomCursor / SetDPIScale / SetCornerRadius
      - AddLeftGroupbox vs AddRightGroupbox (LumenHub cuma punya AddSection
        yang tersusun vertikal dalam satu tab, jadi Auto Farm & Webhook
        sekarang jadi 2 section dalam 1 tab yang sama)
      - Sidebar icon-only (Title kosong) -> sekarang tab pakai nama teks
    Fitur yang digantikan dengan versi bawaan LumenHub:
      - SaveManager (build config section) -> Section:AddConfig({})
        (otomatis handle Save As / Load / Delete / Auto Save / Auto Load /
        Import-Export JSON untuk semua toggle/slider/dropdown/input/keybind)
      - Library.ToggleKeybind (auto toggle menu) -> Keybind manual yang
        toggle Visible dari GUI LumenHub langsung
]]
...
----------------------------------------------------------------
-- COMPATIBILITY SHIM
-- Supaya fungsi inti di bawah yang manggil Library:Notify() /
-- Library.Unloaded / Toggles.X.Value / Options.X.Value TIDAK PERLU DIUBAH.
----------------------------------------------------------------
local Window -- forward declare, dipakai di dalam Library:Unload()
local Toggles = {}
local Options = {}
...
function Library:Notify(cfg)
	Lumen:MakeNotify({
		Title = tostring(cfg.Title or "LumenHub"),
		Content = tostring(cfg.Description or ""),
		Delay = cfg.Time or 3,
	})
end
...
local AutoFarmGroup = Tabs.AutoFarm:AddSection("Auto Farm")
local WebhookGroup = Tabs.AutoFarm:AddSection("Webhook")

----------------------------------------------------------------
-- ================= CORE LOGIC (TIDAK DIUBAH) =================
----------------------------------------------------------------
local BeatState = {
	LastFinishPos = nil,
...
Options.WebhookLink = WebhookGroup:AddInput({
	Title = "Webhook Link",
