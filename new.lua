...
local function SetCompat(container, name, element)
...
ServerHop = function()
	...
	while ToggleValue("ServerHop") and not Library.Unloaded do
		...
		for _, srv in ipairs(res.data) do
			if not ToggleValue("ServerHop") or Library.Unloaded then
				break
			end
			if not forced and not CanHop() then
				break
...
SetCompat(Toggles, "AutoExecute", AutoFarmSection:AddToggle({
    Title = "Auto Execute",
...
SetCompat(Toggles, "EnableWebhook", WebhookSection:AddToggle({
    Title = "Enable Webhook",
    Content = "Enable Discord webhook notifications",
    Default = false,
}))

SetCompat(Options, "WebhookLink", WebhookSection:AddInput({
    Title = "Webhook Link",
...
local ConfigSection = Tabs.Settings:AddSection("Config", true)
