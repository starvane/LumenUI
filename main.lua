...
local function EnsureDefaultConfig()
	if not (writefile and CURRENT_VERSION) then
		return
	end
	local meta = ReadMetaFile(gameName) or {}
	if meta[DEFAULT_CONFIG_FLAG] then
		return
	end
...
function SaveConfig(force)
	if not writefile or not CURRENT_VERSION then
		return false
	end
	if ApplyingConfig and not force then
		return false
	end
...
function QueueSaveConfig(force)
	...
	if ApplyingConfig or not AutoSaveEnabled then
		return false
	end
	if SaveQueued then
		return true
...
local function LoadImageCacheManifest()
	if not (isfile and readfile and isfile(ManifestPath)) then
		return
	end
...
local function SaveImageCacheManifest()
	if ManifestSaveQueued or not writefile then
		return
	end
...
local function TrimImageCache()
	...
	local count = 0
	for _ in pairs(CacheManifest) do
		count = count + 1
	end
...
local function GetWebImageAsset(url)
	...
	if WebImageAssetCache[url] ~= nil then
		return WebImageAssetCache[url]
	end
...
UserInputService.InputEnded:Connect(function(input)
	...
	if handlers then
		...
		if handlers.ended then
			handlers.ended(input)
		end
	end
end)
...
function Chloex:MakeNotify(NotifyConfig)
	...
	task.spawn(function()
		...
		TweenService:Create(NotifyFrame, FadeIn, {
			BackgroundTransparency = 0
		}):Play()
		TweenService:Create(cardScale, FadeIn, {
			Scale = 1
		}):Play()
		TweenService:Create(NotifyStroke, FadeIn, {
			Color = THEME.border,
			Transparency = 0
		}):Play()
		TweenService:Create(TopLine, FadeIn, {
			BackgroundTransparency = 0
		}):Play()
		...
		function NotifyFunction:Close()
			if not NotifyFrame or not NotifyFrame.Parent then
				return
			end
			NotifyStroke:Destroy()
			local FadeOut = TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
			TweenService:Create(NotifyFrame, FadeOut, {
				BackgroundTransparency = 1
			}):Play()
			TweenService:Create(cardScale, FadeOut, {
				Scale = 0.95
			}):Play()
			TweenService:Create(TopLine, FadeOut, {
				BackgroundTransparency = 1
			}):Play()
...
function Chloex:TargetSelector(TargetConfig)
	...
	TargetConfig.Callback = TargetConfig.Callback or function()
	end
	...
	for i, v in ipairs(TargetConfig.Values) do
		if v == TargetConfig.Default then
			selIndex = i
		end
	end
	local MARGIN = 15
	...
	C.activeHover = Color3.fromRGB(
        math.max(0, C.active.R * 255 - 28), math.max(0, C.active.G * 255 - 28), math.max(0, C.active.B * 255 - 28))
	local hoverInfo = TweenInfo.new(0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
	local Selector = {
		_value = TargetConfig.Values[selIndex]
	}
	...
	local function updateBtnVisual(btn)
		...
		if stroke then
			TweenService:Create(stroke, hoverInfo, {
				Color = isActive and C.active or C.stroke
			}):Play()
		end
	end
...
local function RunKeySystemGate(KeyConfig, AccentColor, ScopeName)
	...
	local JunkieReq = (syn and syn.request) or (fluxus and fluxus.request) or (http and http.request) or request or http_request
	...
	local function JunkieGetKeyLink()
		...
		if resp.StatusCode == 429 then
			return nil, "RATE_LIMITTED"
		end
		if resp.StatusCode ~= 200 then
			return nil, resp.Body or "ERROR"
		end
	...
	if keyResizeHandle then
		keyResizeHandle:Destroy()
	end
	...
	local function LayoutKeyUI()
		if not BKey or not BKey.Parent then
			return
		end
		local vp = GetKeyViewport()
		local s = math.min(
            math.min(KeyDesignW, vp.X * 0.92) / KeyDesignW, math.min(KeyDesignH, vp.Y * 0.92) / KeyDesignH)
		if s <= 0 or s ~= s then
			s = 1
		end
		UIScaleKey.Scale = s
	...
	GetKeyBtn.Activated:Connect(function()
		if KeyConfig.GetKeyLink and KeyConfig.GetKeyLink ~= "" then
			if setclipboard then
				...
				than("Link disalin ke clipboard, buka di browser untuk mendapatkan key.", 5, AccentColor, KeyConfig.Title, "Key System")
			else
			...
			task.spawn(function()
				...
				if ok and link then
					if setclipboard then
						...
						than("Link key disalin ke clipboard, buka di browser.", 5, AccentColor, KeyConfig.Title, "Key System")
					else
						than("Link key: " .. tostring(link), 6, AccentColor, KeyConfig.Title, "Key System")
					end
				elseif err and err:match("^RATE_LIMIT") then
					than("Rate limited! Tunggu 5 menit sebelum minta link lagi.", 5, Color3.fromRGB(255, 170, 0), KeyConfig.Title, "Key System")
				else
					than("Gagal mengambil link key: " .. tostring(err or "ERROR"), 5, Color3.fromRGB(255, 90, 90), KeyConfig.Title, "Key System")
				end
			end)
		else
	...
	TweenService:Create(KeyMain, TweenInfo.new(0.18), {
		BackgroundTransparency = 0
	}):Play()
	repeat
		task.wait()
...
function Chloex:Window(GuiConfig)
	...
	if GuiConfig.Search == nil then
		GuiConfig.Search = true
	end
	...
	local function ExtractConfigPayload(payload)
		...
		for key, value in pairs(data) do
			if key ~= "_version" and key ~= "Game" and key ~= "Version" and key ~= "PlaceId" and key ~= "Hub" and key ~= "SavedAt" and key ~= "ActiveConfig" and key ~= "AutoLoad" and not InternalConfigKeys[key] then
				cleaned[key] = value
			end
		end
		return cleaned
	...
	function GuiFunc:SaveConfigAs(name)
		...
		local ok, encoded = pcall(function()
			return HttpService:JSONEncode(GetConfigSnapshot())
		end)
		if not ok then
	...
	function GuiFunc:SetAutoLoad(name)
		...
		EnsureConfigFolder()
		name = name or ""
		writefile(GameConfigFolder .. "/_autoload.json", HttpService:JSONEncode({
			Name = name
		}))
	end
	...
	DropShadowHolder.Position = UDim2.new(0, (BHub.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0, (BHub.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
	...
	if GuiConfig.Discord and GuiConfig.Discord ~= "" then
		...
		DiscordBtn.MouseEnter:Connect(function()
			TweenService:Create(DiscordImg, TweenInfo.new(0.2), {
				ImageColor3 = Color3.fromRGB(120, 132, 255)
			}):Play()
			TweenService:Create(DiscordDivider, TweenInfo.new(0.2), {
	...
	function Tabs:AddTab(TabConfig)
		...
		local function switchToTab(force)
			...
			if not (force or Tab.LayoutOrder ~= currentPage.LayoutOrder) then
				return
			end
			...
			for _, TabFrame in ScrollTab:GetChildren() do
				if TabFrame.Name == "Tab" and token == switchToken then
					TweenService:Create(
                        TabFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.InOut), {
						BackgroundTransparency = 1
					}):Play()
				end
			end
			...
			TweenService:Create(
                Tab, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.InOut), {
				BackgroundTransparency = 0.92
			}):Play()
			TweenService:Create(
                FrameChoose, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
			...
			NameTab.Text = TabConfig.Name
			TweenService:Create(
                FrameChoose, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
			...
			TweenService:Create(
                FrameChoose, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
		...
		function Sections:AddSection(Title, AlwaysOpen)
				...
				task.defer(function()
					...
					pcall(function()
						...
						TweenService:Create(Section, TweenInfo.new(0.5), {
							Size = UDim2.new(1, 1, 0, sectionContentH)
						})
                            :Play()
						TweenService:Create(SectionAdd, TweenInfo.new(0.5), {
							Size = UDim2.new(1, 0, 0, sectionContentH - 38)
						}):Play()
			...
			local function BuildItemsAPI(SectionAdd)
				...
				function Items:AddParagraph(ParagraphConfig)
					...
					local function UpdateSize()
						...
						local ok, bounds = pcall(function()
                            ...
                            ParagraphContent.Text, 12, Enum.Font.Gotham, Vector2.new(maxWidth, math.huge))
						end)
				...
				function Items:AddPanel(PanelConfig)
					...
					function PanelFunc:Set(Value, noSave)
						...
						if shouldSave and not noSave then
							QueueSaveConfig()
						end
					end
				...
				function Items:AddToggle(ToggleConfig)
					...
					ToggleContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
						...
						ToggleContent.Size = UDim2.new(1, - 100, 0, 12 + (12 * (ToggleContent.TextBounds.X // ToggleContent.AbsoluteSize.X)))
						if ToggleConfig.Title2 ~= "" then
							Toggle.Size = UDim2.new(Toggle.Size.X.Scale, Toggle.Size.X.Offset, 0, ToggleContent.AbsoluteSize.Y + 47)
						else
							Toggle.Size = UDim2.new(Toggle.Size.X.Scale, Toggle.Size.X.Offset, 0, ToggleContent.AbsoluteSize.Y + 33)
						end
						ToggleContent.TextWrapped = true
				...
				function Items:AddSlider(SliderConfig)
					...
					SliderContent.Size = UDim2.new(1, - 230, 0, 12 + (12 * (SliderContent.TextBounds.X // SliderContent.AbsoluteSize.X)))
					...
					SliderContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
						...
						SliderContent.Size = UDim2.new(1, - 230, 0, 12 + (12 * (SliderContent.TextBounds.X // SliderContent.AbsoluteSize.X)))
						Slider.Size = UDim2.new(Slider.Size.X.Scale, Slider.Size.X.Offset, 0, SliderContent.AbsoluteSize.Y + 33)
						SliderContent.TextWrapped = true
					...
					local function ApplySliderValue(Value, noSave, instant, fireCallback)
						...
						if instant then
							...
							TweenService:Create(
                            SliderDraggable, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
								Size = targetSize
							}):Play()
						end
						if fireCallback then
					...
					local function FinishSliderDrag(Input)
						...
						TweenService:Create(
                        SliderCircle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				...
				function Items:AddColorPicker(ColorConfig)
					...
					ColorConfig.Callback = ColorConfig.Callback or function()
					end
					...
					local function OpenPalette()
						...
						local function Brighten(c, amt)
							amt = amt or 24
							return Color3.new(
                            math.clamp(c.R * 255 + amt, 0, 255) / 255, math.clamp(c.G * 255 + amt, 0, 255) / 255, math.clamp(c.B * 255 + amt, 0, 255) / 255)
						end
				...
				function Items:AddInput(InputConfig)
					...
					InputContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
						...
						InputContent.Size = UDim2.new(1, - 180, 0, 12 + (12 * (InputContent.TextBounds.X // InputContent.AbsoluteSize.X)))
						Input.Size = UDim2.new(Input.Size.X.Scale, Input.Size.X.Offset, 0, InputContent.AbsoluteSize.Y + 33)
						InputContent.TextWrapped = true
				...
				function Items:AddKeybind(KeybindConfig)
					...
					if isMobile then
						...
						function DummyFunc:Set(kc)
						end
						function DummyFunc:GetValue()
					...
					KeybindConfig.Callback = KeybindConfig.Callback or function()
					end
					...
					KeyBtn.Text = KeybindConfig.Default ~= Enum.KeyCode.Unknown and KeybindConfig.Default.Name or "None"
					...
					local function SetListening(state)
						...
						if state then
							KeyBtn.Text = "..."
							TweenService:Create(KeyBtnStroke, TweenInfo.new(0.2), {
								Color = GuiConfig.Color,
								Transparency = 0
							}):Play()
							...
							local label = KeybindFunc.Value ~= Enum.KeyCode.Unknown and KeybindFunc.Value.Name or "None"
							KeyBtn.Text = label
							TweenService:Create(KeyBtnStroke, TweenInfo.new(0.2), {
								Color = defaultStroke,
								Transparency = 0.2
							...
							TweenService:Create(KeyBtn, TweenInfo.new(0.2), {
								...
								BackgroundTransparency = defaultBgTrans
							}):Play()
							TweenService:Create(KeyBtn, TweenInfo.new(0.2), {
					...
					KeyBtn.MouseEnter:Connect(function()
						if not listening then
							...
							TweenService:Create(KeyBtnStroke, TweenInfo.new(0.15), {
								Color = hoverStroke
							}):Play()
						end
					end)
					...
					UserInputService.InputBegan:Connect(function(input, gpe)
						if not listening then
							...
							if KeybindFunc.Value == Enum.KeyCode.Unknown then
								return
							end
				...
				function Items:AddKeybind2(KeybindConfig)
					if isMobile then
						...
						function DummyFunc:Set(kc1, kc2, noSave)
						end
						function DummyFunc:SetSlot(slot, kc, noSave)
						end
						function DummyFunc:Get(slot)
							return Enum.KeyCode.Unknown
						end
						return DummyFunc
					end
					...
					if shouldSave and type(ConfigData[configKey]) == "table" then
						...
						if saved[1] and Enum.KeyCode[saved[1]] then
							defaultKeys[1] = Enum.KeyCode[saved[1]]
						end
						if saved[2] and Enum.KeyCode[saved[2]] then
							defaultKeys[2] = Enum.KeyCode[saved[2]]
						end
					end
					local KeybindFunc = {
						Value = table.clone(defaultKeys)
					}
					...
					for i = 1, 2 do
						...
						btn.MouseButton1Click:Connect(function()
							...
							if listeningSlot == i then
								slotBtn[i].Text = "..."
								TweenService:Create(slotStroke[i], TweenInfo.new(0.2), {
									Color = GuiConfig.Color,
									Transparency = 0
								}):Play()
								TweenService:Create(slotBtn[i], TweenInfo.new(0.2), {
									BackgroundColor3 = GuiConfig.Color,
									BackgroundTransparency = 0.85
								}):Play()
								TweenService:Create(slotBtn[i], TweenInfo.new(0.2), {
								...
								TweenService:Create(slotBadge[i], TweenInfo.new(0.2), {
									BackgroundColor3 = GuiConfig.Color
								}):Play()
							end
						end)
					...
					local function RefreshSlots()
						for i = 1, 2 do
							...
							TweenService:Create(slotStroke[i], TweenInfo.new(0.15), {
								Color = defaultStroke,
								Transparency = 0.2
							}):Play()
							TweenService:Create(slotBtn[i], TweenInfo.new(0.15), {
								BackgroundColor3 = defaultBg,
								BackgroundTransparency = defaultBgTrans
							}):Play()
							TweenService:Create(slotBtn[i], TweenInfo.new(0.15), {
							...
							TweenService:Create(slotBadge[i], TweenInfo.new(0.15), {
								BackgroundColor3 = Color3.fromRGB(80, 82, 90)
							}):Play()
						end
					end
					local function ParseKey(kc)
						if type(kc) == "string" then
							local ok, conv = pcall(function()
								return Enum.KeyCode[kc]
							end)
					...
					function KeybindFunc:Set(kc1, kc2, noSave)
						...
						if shouldSave then
							ConfigData[configKey] = {
								KeybindFunc.Value[1].Name,
								KeybindFunc.Value[2].Name
							}
							if not noSave then
								QueueSaveConfig()
							end
						...
						if listeningSlot == 0 then
							...
							for i = 1, 2 do
								...
								if bound ~= Enum.KeyCode.Unknown and bound == input.KeyCode then
									...
									if not ok then
										warn("Keybind2 Callback error:", err)
									end
									break
								end
							end
				...
				function Items:AddTargetSelector(TargetCfg)
					...
					local Options = TargetCfg.Options or TargetCfg.Values or {
						"KILLER",
						"SURVIVOR",
						"ZOMBIE"
					}
					...
					local Callback = TargetCfg.Callback or function()
					end
					local Selector
					local Dropdown = Items:AddDropdown({
						...
						Callback = function(v)
							Callback(v)
							if Selector and v ~= Selector:Get() then
								Selector:Set(v)
							end
						end,
					})
					Selector = Chloex:TargetSelector({
						...
						Callback = function(v)
							...
							Callback(v)
							if Dropdown and v ~= Dropdown.Value then
								Dropdown:Set(v, true)
							end
						end,
					})
				...
				function Items:AddConfig(ConfigCfg)
					...
					Items:AddButton({
						...
						SubCallback = function()
							if selectedConfig then
								GuiFunc:LoadConfigByName(selectedConfig)
							end
						end,
					})
					Items:AddButton({
					...
					AutoSaveToggle = Items:AddToggle({
						...
						Callback = function(value)
							...
							if initializingAutoSaveToggle then
								return
							end
							if value then
					...
					AutoToggle = Items:AddToggle({
						...
						Save = false,
						Callback = function(value)
							if initializingAutoToggle then
								return
							end
				...
				function Items:AddCardWidget(CardConfig)
					...
					RegisterSearch({
						label = CardConfig.Title or "Card Widget",
						tab = TabConfig.Name,
						kind = "CardWidget",
						section = self,
						switch = SearchSwitch
				...
				function Items:AddHStack(StackConfig)
					...
					if Sizing == "Equal" then
						local function RecalculateWidths()
							...
							for _, child in ipairs(contentChildren) do
								child.Size = UDim2.new(1 / count, extraOffset / count, child.Size.Y.Scale, child.Size.Y.Offset)
							end
						end
						local widthsPending = false
						local function ScheduleRecalculateWidths()
							if widthsPending then
								return
							end
							widthsPending = true
							task.defer(function()
				...
				function Items:AddPresetManager(PresetConfig)
					...
					PresetConfig.Callback = PresetConfig.Callback or function()
					end
					...
					OptHeader.MouseButton1Click:Connect(function()
						...
						if isOptOpen then
							OptArrow.Rotation = 0
							TweenService:Create(OptContainer, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
								Size = UDim2.new(1, 0, 0, OptLayout.AbsoluteContentSize.Y)
							}):Play()
						else
							OptArrow.Rotation = - 90
							TweenService:Create(OptContainer, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
					...
					NameBox.FocusLost:Connect(function()
						currentPresetName = NameBox.Text
					end)
					...
					local function CreateBtn(text, bgColor, callback)
						...
						Btn.MouseEnter:Connect(function()
							TweenService:Create(Btn, TweenInfo.new(0.15), {
								BackgroundColor3 = bgColor:lerp(Color3.fromRGB(255, 255, 255), 0.1)
							}):Play()
						end)
						Btn.MouseLeave:Connect(function()
							TweenService:Create(Btn, TweenInfo.new(0.15), {
			...
			function Items:GetOpen()
				return SectionHandle.open
			end
	...
	GuiFunc.InfoTab = function(_, cfg)
		return Tabs:InfoTab(cfg)
	end
	Tabs.Window = GuiFunc
	Tabs.ExportConfig = function()
		return GuiFunc:ExportConfig()
	end
	Tabs.ImportConfig = function(_, str)
		return GuiFunc:ImportConfig(str)
	end
	if GuiConfig.Search then
		UserInputService.InputBegan:Connect(function(input, gpe)
			if gpe then
			...
			local ctrl = UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.RightControl) or UserInputService:IsKeyDown(Enum.KeyCode.LeftMeta) or UserInputService:IsKeyDown(Enum.KeyCode.RightMeta)
			local shift = UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or UserInputService:IsKeyDown(Enum.KeyCode.RightShift)
			if ctrl and shift and input.KeyCode == Enum.KeyCode.F then
				if DropShadowHolder then
					DropShadowHolder.Visible = true
				end
				if GuiFunc.FocusSearch then
					GuiFunc.FocusSearch()
				end
